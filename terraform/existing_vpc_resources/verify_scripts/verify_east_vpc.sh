...
print_section "EAST VPC VERIFICATION"
...
if ! is_tfvar_true "enable_build_existing_subnets" "$TFVARS_FILE"; then
    print_skip "East VPC is not enabled (enable_build_existing_subnets = false)"
    print_summary
    exit 0
fi
...
VPC_NAME="${CP}-${ENV}-east-vpc"

print_info "Expected East VPC CIDR: $VPC_CIDR_EAST"

# 1. Verify VPC exists
print_info "Checking if East VPC exists..."
VPC_ID=$(verify_vpc_exists "$VPC_NAME" "$AWS_REGION")
if [ $? -eq 0 ]; then
    print_pass "East VPC exists: $VPC_ID"
else
    print_fail "East VPC does not exist: $VPC_NAME"
    print_summary
    exit 1
fi
...
if verify_vpc_cidr "$VPC_ID" "$VPC_CIDR_EAST" "$AWS_REGION"; then
    print_pass "VPC CIDR matches: $VPC_CIDR_EAST"
else
    result=$(verify_vpc_cidr "$VPC_ID" "$VPC_CIDR_EAST" "$AWS_REGION")
...
for AZ_NUM in 1 2; do
    ...
    PUBLIC_SUBNET_NAME="${CP}-${ENV}-east-public-az${AZ_NUM}-subnet"
...
for AZ_NUM in 1 2; do
    ...
    TGW_SUBNET_NAME="${CP}-${ENV}-east-tgw-az${AZ_NUM}-subnet"
...
MAIN_RT_ID=$(aws ec2 describe-route-tables \
    --region "$AWS_REGION" \
    --filters "Name=vpc-id,Values=${VPC_ID}" "Name=tag:Name,Values=${CP}-${ENV}-east-vpc-main-route-table" \
...
if [ "$MAIN_RT_ID" != "None" ] && [ -n "$MAIN_RT_ID" ]; then
    ...
    # Verify local route
    if verify_route_exists "$MAIN_RT_ID" "$VPC_CIDR_EAST" "$AWS_REGION"; then
        print_pass "Local route to VPC CIDR exists: $VPC_CIDR_EAST"
    else
        print_fail "Local route to VPC CIDR not found: $VPC_CIDR_EAST"
    fi
...
if [ $? -eq 0 ]; then
    ...
    if [ $? -eq 0 ]; then
        ...
        print_fail "TGW attachment not found for east VPC"
    fi
else
    print_fail "Transit Gateway not found: $TGW_NAME"
fi

# 6. Verify TGW route table for East VPC
print_info "Verifying TGW route table for East VPC..."

TGW_RT_NAME="${CP}-${ENV}-east-tgw-rtb"
...
if [ "$TGW_RT_ID" != "None" ] && [ -n "$TGW_RT_ID" ]; then
    ...
    # Verify route table is associated with east attachment
    ...
    if [ "$ASSOCIATED" == "associated" ]; then
        print_pass "TGW route table is associated with East VPC attachment"
    else
        print_fail "TGW route table is not associated with East VPC attachment: $ASSOCIATED"
    fi
...
if is_tfvar_true "enable_linux_spoke_instances" "$TFVARS_FILE"; then
    ...
    for AZ_NUM in 1 2; do
        INSTANCE_NAME="${CP}-${ENV}-east-public-az${AZ_NUM}-instance"
        ...
        if [ $? -eq 0 ]; then
            ...
            if [ $AZ_NUM -eq 1 ]; then
                SUBNET_NAME="${CP}-${ENV}-east-public-az1-subnet"
            else
                SUBNET_NAME="${CP}-${ENV}-east-public-az2-subnet"
            fi
            ...
            # Verify NO public IP (East VPC has no IGW, only TGW connectivity)
